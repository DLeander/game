...
Texture::Texture(unsigned char* texture, GLenum type, GLenum slot, GLenum format, GLenum pixelType){
    // Load the player's texture

    // Texture Variables
    m_Type = type;
    int iTextureWidth=128, iTextureHeight=128;
    stbi_set_flip_vertically_on_load(true);
    ...
    glTexImage2D(type, 0, GL_RGBA, iTextureWidth, iTextureHeight, 0, format, pixelType, texture);
    glGenerateMipmap(type);
